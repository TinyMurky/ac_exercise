...
function addFriend(id, target=undefined) {
  ...
  if (getPaginatorPage(now_is_page, targetList).length === 0) {
    now_is_page -= 1
  }
  console.log(now_is_page)
  renderUserCard(getPaginatorPage(now_is_page,targetList), dataPanel)
  renderPaginator(targetList)
...
function findMyFriend(searchKeyword) {
  const friendInFriendList = allUser.filter(user => {
    return myFriend[user.id]
  }) 
  const friendFound = friendInFriendList.filter((user) => {
...
//幫入取得各分頁要畫的畫面
function getPaginatorPage(page, userArray){
  const friendInFriendList = userArray.filter(user => {
    return myFriend[user.id]
  }) 
  const pageBegin = (page - 1) * CARD_PER_PAGE
  const pageEnd = pageBegin + CARD_PER_PAGE

  return friendInFriendList.slice(pageBegin, pageEnd)
}

//render Paginator
function renderPaginator(userArray) {

  const friendInFriendList = userArray.filter(user => {
    return myFriend[user.id]
  }) 
  const pageWeNeedToPrint = Math.ceil(friendInFriendList.length/ CARD_PER_PAGE)
  let pageItem = ""
...
//把allUser中所有的資訊都畫出來
function renderUserCard(userArray, dataPanel) {
  ...
  friendInFriendList.forEach(user => {
